...
CockroachDB × AWS Agentic Memory Hackathon project (deadline 2026-08-19). An AI health companion whose core product is persistent, lifelong memory: a custom **Memory Engine** on CockroachDB (typed JSONB memories + `VECTOR(512)` embeddings, hybrid SQL+vector retrieval, deterministic EvidenceTraces driving a glass-box UI), a model-agnostic LangGraph agent (Bedrock default), FastAPI + Vite/React in one Docker image on AWS App Runner. Standard multi-user SaaS — every account starts with empty memory.
