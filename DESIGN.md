...
confined to the same right two-thirds (`md` and up), sits `Particles`
(`components/effects/Particles.tsx`) — a slow drifting point field, also `pointer-events-none` —
closing the gap between a single centered mark and how much space the column actually has.
