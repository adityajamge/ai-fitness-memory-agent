...
**App (wireframe v8 — timeline returns to Chat full-bleed, 2026-08-18, §16; sidebar scoped to
Chat only, 2026-08-13 IA revision), desktop ≥1280px. This is Chat's layout; Review and Profile
are the single-column layout described in §6.20/§6.19, with nothing to either side:**

```
┌───────────────────────────────────────────────────────────┐
│ top bar                    56px   mark · Chat Review Profile│
├───────────────────────────────────────────────────────────┤
│ timeline strip             72px   full bleed, 60 days      │
├────────────┬────────────────────────────────┬──────────────┤
...
**The timeline spans the full window on Chat, edge to edge, above all three columns.** It was
scoped to Review only by the 2026-08-13 revision, returned to Chat on 2026-08-18 inset inside the
conversation column, and was moved to full-bleed the same day on explicit instruction (§16). The
concern the inset version answered — that a full-width band reads as a global header and outranks
the answer — is addressed by the strip's own restraint rather than by its width: 72px of `--faint`
bars on the graph rule, no numbers, no labels, and the only date in it revealed on hover. Chat's
window is **60 days**; Review keeps the default 90 (§6.8). Profile has no timeline.
...
**Two mounts** (2026-08-18, §16): **Chat**, full-bleed under the top bar across the whole window,
showing **60 days** without scrolling; and **Review**, full column width, showing **90**. Not a
rail on every screen — Profile has none. The day count is the component's one per-mount option
(`visibleDays`); everything else about the rail is identical, and both scroll left into older
history with `now` pinned right. Bar width is `railWidth / visibleDays`, so a wider viewport buys
wider bars, never more history — the window is a span of time, not a bar count.
...
```
[mark] AyuMind AI                         Logo 36px + text-dense font-medium
                                          (matches the landing nav exactly, added
                                           2026-08-16 — was mark-only, no visible
                                           wordmark, before)
...
**Right column, `lg` and up (added 2026-08-16, §16 Decisions Log).** `VitalsFigure`
(`components/effects/VitalsFigure.tsx`) — the spinning-ring/ECG-trace mark ported from a
design-tool export — sits centered in the space the graph rule alone used to fade into,
`pointer-events-none` and `aria-hidden`, sized off the viewport (`min(78vmin, 620px)`, matching
the source export) rather than a fixed cap that read as too small for the column once seen
running. Hidden below `lg` (1024px) rather than the graph rule's own `md` (768px) breakpoint, so
the two never compete for room at in-between widths. Behind both the figure and the graph rule
sits `Particles` (`components/effects/Particles.tsx`) — a slow drifting white starfield, also
`pointer-events-none` — spanning the full page (`md` and up, revised 2026-08-16 from an initial
right-column-only pass) rather than stopping at the graph rule's own boundary.
Purely decorative; every piece of information on this screen still lives in the form.
...
| ~~Photo-ingest UI~~ **Superseded 2026-08-14** | Was: **Not** this document's M7 (the timeline strip, shipped) — Phase 5's separately-numbered M7 (photo ingestion, the backend feature) was cut for the hackathon. Reopened at the user's explicit request: the composer now has a photo-attach affordance (§6.2, extended) feeding the reintroduced M7 (ephemeral-storage variant — no S3; `TODOS.md`). See §16 Decisions Log, 2026-08-14. |
...
| 2026-08-16 | **`VitalsFigure` added to the auth screens' right column, breaking rules 6 and 12** | Explicit user instruction, source provided as a design-tool export (`Neon Vitals Object.dc.html` + `assets/figure.png` — ported into `components/effects/VitalsFigure.tsx`/`.css` and removed from `public/` once ported, same precedent as `Logo.tsx`: the export and its `support.js` templating runtime were never meant to be served). A spinning ring (soft/mid/crisp blur layers via `<use>`), a pulsing radial halo, a heartbeat trace whose bright dot sweeps the path (`offset-path`) on a loop, a heartbeat-opacity ground shadow, and a slow whole-mark drift. Hardcoded hex (`#6fe8dc`/`#2ee0d8`/`#1a8f9d`/`#e8fff6`) rather than tokens because the accent has to match the source raster figure's own cyan glow, which cannot be recolored — the same trade-off `Logo.tsx` and `MoltenMetal.tsx` already made, scoped here to §6.17's right column only (`lg` and up), `pointer-events-none`, `aria-hidden`. `useId()` scopes each instance's filter/gradient/`<use>` ids, matching `Logo.tsx`, so it stays safe to drop into more than one screen later. Reduced motion is ANDed with the `animated` prop, never bypassed by it (rule 14 upheld, same as every other animated component). |
| 2026-08-16 | **`Particles` (React Bits) added behind the auth screens' right column; `VitalsFigure` resized from a fixed 420px to `min(78vmin, 620px)`** | Explicit user instruction, after seeing the shipped `VitalsFigure` read as a small medallion adrift in a mostly-empty column — the ratio was unbalanced, not the mark itself. Two fixes, same session: `VitalsFigure`'s cap changed from `--container-pane` (a coincidental 420px match, not a semantic reuse — this mark isn't the evidence pane) to a viewport-relative size matching the source export's own sizing, so it scales with the column instead of stranding a fixed box inside it. Second, `components/effects/Particles.tsx`/`.css` — a `react-bits` WebGL point-cloud background, source pasted directly by the user, ported to TypeScript on the same terms `MoltenMetal.tsx` already established for this exact library (§16, 2026-08-09): explicit instruction, hardcoded colors (rule 6/12), `react-bits` itself (rule 11, still not a precedent elsewhere — both entries now scope it to decorative, non-content backgrounds only, landing hero and this column). Ported with `MoltenMetal`'s own performance/accessibility discipline even though upstream `Particles` has neither: `prefers-reduced-motion` skips opening the WebGL context at all (`ogl`, already a dependency), and the render loop pauses via the same `IntersectionObserver`/`visibilitychange` pattern rather than running `requestAnimationFrame` off-screen or backgrounded. Default `particleColors` swapped from upstream's plain white to the same `#6fe8dc`/`#2ee0d8`/`#1a8f9d` cyan family `Logo.tsx`/`VitalsFigure.tsx` already use — white points on `--color-background` is the generic-AI-particle-hero look §11 exists to avoid; this keeps it inside the instrument palette already established. Confined to the same right two-thirds as the graph rule (`md` and up) and layered behind both the graph rule and `VitalsFigure`, `pointer-events-none` — the form column itself is untouched, so §6.17's "the form sits directly on `--background`" still holds. |
| 2026-08-16 | **`VitalsFigure`'s ground-shadow ellipse removed** | Explicit user instruction ("the logo shadows is not looking good... the rectangle under logo"), after seeing a visible glowing bar beneath the figure. Traced to pixel data, not guessed: `vitals-figure.png`'s own alpha channel was checked directly (decoded the PNG's IDAT stream and sampled it) and fades smoothly to fully transparent at the bottom — the raster asset was not the source. The actual cause was the ported ellipse standing in for a ground shadow (`cx="200" cy="358" rx="86" ry="6"`, `stdDeviation="7"` blur): thin and mostly-opaque enough that the blur read as a flat glowing bar rather than a soft glow, a flaw the source export's own dark radial-gradient backdrop (never reused here) had been hiding. Cut rather than re-tuned — the ring/torso/ECG-sweep mark reads as a complete idea without it, and DESIGN.md's own restraint rule (no shadows outside `--shadow-overlay`/`--shadow-popover`) argues for removing an ad hoc one rather than tuning it into an exception. `vf-beat`'s now-unused keyframes were deleted from `VitalsFigure.css` in the same pass. |
| 2026-08-16 | **`Particles` revised: white instead of the brand cyan, full page instead of the right column only** | Explicit user instruction, twice in the same session: first that the field read as too sparse ("the stars are so much less... so much small" — `particleCount` 110→200, `particleBaseSize` 60→90, `opacity` 70%→85%, `particleSpread` pulled in from 13→6 since the wide spread was pushing most points deep enough on the z-axis to shrink past visibility, so raising density alone wouldn't have fixed it), then that it should be white rather than green/teal and cover the whole screen, not just the right two-thirds. Both reverse choices this same file's Decisions Log made hours earlier for reasons that turned out not to hold once seen running: the cyan default (picked to avoid a "generic white AI particle hero") and the right-column confinement (picked to keep the form column matching §6.17's "no `--surface` panel" reading) both lose to direct instruction on how the shipped screen should actually look. `particleColors={["#ffffff"]}` is passed at the `AuthScreen.tsx` call site rather than changed on `Particles`' own default, so the component's default (still the cyan family, still documented above) stays available for any other decorative use. The wrapping div drops `right-0 w-2/3` for a plain `inset-0`, `md` and up; mobile is still hidden, unchanged. |
| 2026-08-16 | **Auth screens brought back into the same visual identity as the landing page — `/design-review`** | User feedback, comparing screenshots of the two: "navigating from landing page to signup and sign in... it is looking different website, not same website page." Two root causes, both fixed, neither a new deviation from the rules already logged above. First: the auth mark was icon-only (`<Logo size={20} />`, no visible text, an `aria-label` carrying the name instead) while the landing nav shows `Logo size={36}` next to a visible `text-dense font-medium` "AyuMind AI" span — reasonable for the footer's icon-only precedent (sits low on the page, next to other text) but wrong for auth's placement, which functions as this page's header the same way the nav's does. `AuthScreen.tsx` now matches the nav treatment exactly, and §6.17's anatomy diagram (above) is corrected to match — it still described the pre-Logo.tsx-swap mark. Second, and larger: `VitalsFigure` drew its ring, ECG trace, and halo from a single flat `#6fe8dc` — a cyan lifted from the source export's own figure, unrelated to `Logo.tsx`'s actual blue→teal→green gradient family (`mainId`/`coolId`/`warmId`). Recolored to reuse those exact three gradients (copied hex-for-hex, not re-derived), so the auth mark now reads as the brand mark's bigger sibling instead of a fourth, unrelated hue introduced just for this screen. `vitals-figure.png` itself still can't be recolored (raster asset), so its `drop-shadow` tint moved to `#1fc9a6` — the ramp's own middle stop — so its glow at least matches the family the ring is drawn in. Both fixes stay inside the rule 6/12 exception this component already carries; no new exception, no widened scope. Verified live, dark theme, `/login` and `/signup`, desktop and mobile, against a fresh landing-page screenshot side by side. |
...
| 2026-08-18 | **Chat's timeline moved from inset-in-the-conversation-column to full-bleed across the window — same day, second placement** | Explicit user instruction on seeing the inset version shipped ("i dont want timeline frame only in chat section, i want it to fill whole horizontal screen"), correcting the interpretation call flagged when the entry above was written. The strip now sits directly under the top bar and above the row holding the thread rail, the conversation and the engine pane — the pre-2026-08-13 geometry, reached deliberately this time rather than by default. The inset placement existed to answer one concern: that a full-width band reads as a global header and outranks the answer (rank 1 of §9's hierarchy). That concern is real but is answered by the strip's own restraint rather than by its width — 72px of `--faint` bars on the graph rule, no numbers, no labels, no title, and the only date in the entire app revealed on hover — and one continuous edge-to-edge rail expresses "this is your whole history" better than a rail that starts and stops at two arbitrary column edges. Only the mount point moved; the component, its 60-day window, the scroll behavior, the day-view click and every state are unchanged, and Review's mount is untouched. Bar width is `railWidth / visibleDays`, so the wider rail spends the extra room on wider bars rather than silently showing more days than the 60 that were asked for. |
