...
| [frontend-guidelines.md](frontend-guidelines.md) | **Locked** engineering contract for `web/` (Phase 6 M4–M8): the layout, the token layer, the two-typefaces-as-provenance rule and how it gets broken, the fetch boundary and why hydration is batched, the four states every data component ships, motion under `LazyMotion strict`, accessibility obligations, the review checklist, and an honest note on which rules tooling cannot enforce. Companion to [DESIGN.md](../../DESIGN.md), which owns appearance |
